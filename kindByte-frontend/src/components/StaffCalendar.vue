    ...
    <template v-else>
      ...
      <div class="section">
        <div class="calendar-card">
          ...
          <div class="calendar-grid">
            ...
            <div v-for="date in calendarDates" :key="date.key" :class="['calendar-date', {
      ...
      <div class="section">
        <div class="section-header">
          ...
          <span class="count-badge">{{ filteredActivities.length }} Event{{ filteredActivities.length !== 1 ? 's' : ''
            }}</span>
        </div>
        ...
        <div v-else class="event-list">
          <div v-for="activity in filteredActivities" :key="activity.id" class="event-card"
            ...
            <div class="event-content">
              ...
              <p class="event-meta">{{ formatTime(activity.startTime) }} - {{ formatTime(activity.endTime) }} • {{
                activity.location }}</p>

              <div class="event-status">
                <span class="status-badge participants">
                  👥 {{ getTotalJoined(activity) }}/{{ activity.maxCount }} Participants
                </span>
                <span :class="['status-badge', checkVolunteerRatio(activity) ? 'volunteers-ok' : 'volunteers-alert']">
                  🤝 {{ getVolunteerCount(activity) }} Vol{{ getVolunteerCount(activity) !== 1 ? 's' : '' }} (1:{{
      ...
      <div v-if="lowVolunteerEvents.length > 0" class="section">
        ...
        <div class="alert-list">
          <div v-for="event in lowVolunteerEvents.slice(0, 3)" :key="event.id" class="alert-card">
            <div class="alert-info">
              <strong>{{ event.title }}</strong>
              <span>{{ formatShortDate(event.startTime) }} • Only {{ getVolunteerCount(event) }} volunteer{{
                getVolunteerCount(event) !== 1 ? 's' : '' }}</span>
            </div>
